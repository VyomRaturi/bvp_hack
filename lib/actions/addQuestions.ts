...
export const addQuestionsToHackathon = async (payload: AddQuestionsPayload): Promise<CreatedQuestion[]> => {
    ...
    questions.forEach((q, index) => {
        ...
        q.ans.forEach((a, ansIndex) => {
            if (!a.answer || typeof a.answer !== "string") {
                throw new Error(`Invalid or missing 'answer' at question index ${index}, answer index ${ansIndex}.`);
            }

            if (typeof a.score !== "number") {
                throw new Error(`Invalid or missing 'score' at question index ${index}, answer index ${ansIndex}.`);
            }
        });
    });
...
export const getQuestionsFromParameters = async (parameters: string[], remarks?: string): Promise<QuestionInput[]> => {
    const res = await fetch('https://api.hyperleap.ai/prompts', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
    ...
    const result = await res.json();
    const content = JSON.parse(result.choices[0].message.content) as QuestionInput[];
    return content;
}
